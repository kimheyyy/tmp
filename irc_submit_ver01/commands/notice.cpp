...
int Command::notice(const Client &sender, const std::list<Client> &cList)
{
    ...
    send_fd(_conCit->get_fd(), RPL_NOTICE(sender.get_nick(), sender.get_nick(), sender.get_servername(), cName, _splitMsg[2]));
    return (1);
}
...
int Command::notice(const Client &sender, const std::list<Channel> &chList)
{
    ...
    if (!channel.get_allowMsg())
        ...
        else
        {
            ...
            send_all(fds, RPL_NOTICE(sender.get_nick(), sender.get_nick(), sender.get_servername(), chName, _splitMsg[2]));
            return (1);
        }
    else
    {
