...
void Command::split_msg(void)
{
    ...
    if (_msg[start] == ':')
    {
        start = _msg.find(" ", start);
        if (start != std::string::npos)
            start++;
        else
            return; // 공백이 없는 경우 함수 종료
    }
    ...
    while (start < _msg.length() && start != std::string::npos)
    {
        ...
        // 콜론이 발견되면, 콜론 이후의 모든 문자열을 파라미터로 처리
        if (_msg[start] == ':')
        {
            end = _msg.find("\r\n", start);
    ...
    {
        std::cout << "Split Message [" << i << "]: " << _splitMsg[i] << std::endl;
    }
}
...
int Command::check_nick(std::string curName, std::string nickName, std::list<Client> &cList, int cFd)
{
	...
	{
		send_fd(cFd, ERR_NICKNAMEINUSE(curName, nickName));
		return (-1);
	}
